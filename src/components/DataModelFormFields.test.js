...
const DataModelFormFields = ({process, updateProcess}) => {
        ...
        useEffect(() => {
                process.attributes["dascontract:data-model"] && setFormData(JSON.parse(process.attributes["dascontract:data-model"]));
                            ...
                            properties: {
                                    ...
                                    items: {
                                        ...
                                        properties: {
                                            ...
                                            dataType: {
                                                type: "string",
                                                title: "Data type",
                                                enum: ["Number", "Collection of numbers", "UnsignedNumber",
                                                    "Positive number", "Collection of positive numbers", "Boolean", "Collection of booleans",
                                                    "Text", "Collection of texts", "Date and time", "Address", "Collection of addresses", "Billable address",
                                                    "Collection of billable adresses", "Binary array (file, ...)", "Collection of binary arrays (files, ...)"
                                                ],
                                            },
                                        }
                                    },
                                ...
                                "referenceProperties": {
                                    ...
                                    items: {
                                        ...
                                        "required": ["id", "name", "propertyRefenceType", "targetEntityType"]
                                    }
                                }
                            },
                            "required": ["id", "name"]
        ...
        return (
            <Col sm="6">
                <Accordion className="accordion-inline">
                    <Accordion.Item eventKey={`${process.attributes && process.attributes.id}`}>
                        <Accordion.Header><FontAwesomeIcon className="mx-2"
                                                           icon={faUser}/>{process.attributes && process.attributes.name} ({process.attributes && process.attributes.id})
                        </Accordion.Header>
                        <Accordion.Body>
                            ...
                            <FormDataStateContext.Provider value={[formData, setFormData]}>
                                <SchemaForm schema={schema} uiSchema={uiSchema}
                                    ...
                                    <Button
                                        ...
                                        size="m" onClick={() => updateProcess(process, JSON.stringify(formData))}>
