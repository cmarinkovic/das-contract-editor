...
import {faDownload, faRedo, faSave, faTable, faUndo} from "@fortawesome/free-solid-svg-icons";
...
const BusinessRuleActivity = ({task, updateTask}) => {
    ...
    useEffect(() => {
        if (task.attributes["dascontract:activity-properties"]) {
            const xmlModel = convert.json2xml(JSON.parse(task.attributes["dascontract:activity-properties"]), {
                compact: false,
                spaces: 4
            })
    ...
    const openDmn = async dmnXML => {
        await dmnModelerRef.current.importXML(dmnXML).then(({warnings}) => {
            if (warnings.length) {
    ...
    const saveModel = async () => {
        try {
            dmnModelerRef.current.saveXML().then(
                ({xml}) => {
                    const jsonModel = JSON.stringify(convert.xml2json(xml, {
                        compact: false,
                        spaces: 4
                    }));
    ...
    return (
        <Col sm="12" {...getRootProps({className: "dropzone"})}
            /*onKeyDown={handleKeyDown}*/>
            <input {...getInputProps()} />

            <SaveChangesModal context="replaceDiagram" toggleModalButtonRef={toggleModalButtonRef}
            ...
            <Accordion className="accordion-inline">
                <Accordion.Item eventKey={`${task.attributes && task.attributes.id}`}>
                    <Accordion.Header><FontAwesomeIcon className="mx-2"
                                                       icon={faTable}/>{task.attributes && task.attributes.name} ({task.attributes && task.attributes.id})
                    </Accordion.Header>

                    <Accordion.Body>
                        <Resizable height={viewerHeight} onResize={handleResize}>
                            <Row className={`dmn-container ${importError && "with-import-error"} ${
                                ...
                                <Col>
                                    ...
                                    <div className="message import-error">
                                        <div className="px-4 py-5 my-5 text-center">
                                            <div className="col-lg-6 mx-auto">
                                                <p className="lead mb-4 note">
                                                    Ooops, we could not display the DMN model.
                                                    Drop another file or <span
                                                    className="link-primary"
                                                    style={{cursor: "pointer"}}
                                                    onClick={createNewDmn}
                                                > create a new DMN model</span>.
                                    ...
                                    <div ref={canvasRef} className="dmn-canvas"
                                         style={{height: viewerHeight * 0.5 + 'px'}}/>
                                </Col>
                            </Row>
                        </Resizable>
                        ...
                        <ButtonGroup
                            className={`${(!successfulImport || importError) && "d-none"} dmn-save-buttons m-2`}>
                            <Button
