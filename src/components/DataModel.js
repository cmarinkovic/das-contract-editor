...
const DataModel = ({appStarted, loadedContract, loadContract, loadContractError}) => {
    //Data load
    ...
    return (
        <Container className={`${loadContractError && "d-none"}`} fluid={true}>
            ...
            <Row className="mt-2">
                {modelElements && modelElements[0].map(process => {
                    return <DataModelFormFields key={process.attributes.id} process={process}
